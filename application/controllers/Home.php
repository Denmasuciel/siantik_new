   ...
   public function __construct()
   {
	...
	Public function racikan_excel()//ok
	{
		...
		$tahun=date("Y-m-d");	
			...
			$d['rata'] = $this->db->query("select round((SUM(menit) /COUNT(*)),2) as rata FROM vw_hitung_waktu where tanggal='$tahun' and loket='A'")->row()->rata;
	...
	Public function nonracikan_excel()//ok
	{
		...
		$tahun=date("Y-m-d");	
			...
			$d['rata'] = $this->db->query("select round((SUM(menit) /COUNT(*)),2) as rata FROM vw_hitung_waktu where tanggal='$tahun' and loket='B'")->row()->rata;
